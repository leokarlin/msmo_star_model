function [ res ] = vote_ms3( model, img, params, test_descrs, pos_xy )
...
%             w=un_probs(:,validCols);
            ...
            [tMin,nnClass]=min(d);
            all_cands=[all_cands ; voting_max_coord(1:2) voting_max_val cand_scale(iCand) nnClass];
        end
    else
        %% vote for classes
...
if params.debugFlag
    ...
    imshow(img);
    hold on;
    plot(all_cands(:,1)/working_res,all_cands(:,2)/working_res,'rx','markersize',15);
end
